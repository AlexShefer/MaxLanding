export const content = {
    rus: {
        greeting: ["Привет! Меня зовут", "Максим Лалак."],
        about: "На этом сайте я рассказываю о направлениях работы которыми занимаюсь, а также, делюсь результатами этой  деятельности. Если ознакомившись с этой информацией у вас возникнет желание сотрудничать, обязательно свяжитесь со мной любым удобным способом.",
        education:
            "Российкий Государственный Университет Туризма и Сервиса, Юрист, 2005-2010",
        experience: [
            "Опыт работы по специальности: Инхаус-юрист, 2012-2014",
            "Опыт работы по другим направлениям: Радиоведущий, 2015-2016; Преподаватель на курсах в средней школе, 2008-2010",
            "Собственный бизнес: руководство проектами, консультирование в сфере предпринимательства, полный цикл создания интерактивных развлекательных проектов. ",
        ],
        occupation: {
            music: {
                myProjects: [
                    {
                        img: "projects/wavemeup.jpeg",
                        text: " WaveMeUp (metagrunge)",
                        link: "https://www.youtube.com/watch?v=spCX6BueUvo",
                    },
                    {
                        img: "projects/paranoid.jpg",
                        text: "Paranoid Park (chaotic hardcore)",
                        link: "https://www.youtube.com/watch?v=vRdtmslc_Xo",
                    },
                    {
                        img: "projects/raisdem.png",
                        text: "raizdem (electronic), KAROSHI (deftones core)",
                        link: "https://www.youtube.com/watch?v=ymzJwEoTDP4",
                    },
                    {
                        img: "projects/october.jpeg",
                        text: "Однажды в Октябре (emo, pop-punk).",
                        link: "https://www.youtube.com/watch?v=UIMSESUqs5E",
                    },
                ],

                description:
                    "Исполняю музыку вживую, но часть проектов являются студийными. Делаю записи самостоятельно, стараюсь обучаться, работать над улучшением качества и повышением уровня собственных навыков в этом направлении.",

                podcast: [
                    {
                        name: "Подкаст «Голосовые норм?»",
                        link: "https://music.yandex.ru/album/25333859",
                    },

                    {
                        name: "YouTube канал Dmitry More, с аудиторией более 100 000 человек",
                        link: "https://www.youtube.com/@DmitryMore. ",
                    },
                ],
            },
            copywriting: [
                "Работаю в проекте «Хэнговер», это видеоблог и паблик в социальных сетях с суммарной аудиторией 35 000 человек. Пишу тексты о выходящих релизах и на другие темы связанные с музыкой, литературой и кино. Кроме того, занимаюсь подборкой новостей и помощью с подготовкой сценариев для еженедельных подкастов. ",
                "Пишу тексты для блога е-коммерс компании Azalia Now.",
            ],
            testing: [
                "Занимаюсь ручной тестировкой сайтов, а также тестировкой мобильных приложений для Apple через TestFlight. Составляю необходимую документацию, веду своевременную отчётность, в соответствии с требованиями компании",
            ],
        },
        contact: [
            "По всем представленным направлениям я стараюсь развиваться и постоянно повышать качество работы; Всегда ищу  креативный и творческий подход, стараюсь создавать новое. Увлекаюсь и отдаюсь делу на 100%.",
            "Если вы хотели бы посотрудничать, готов рассмотреть любые варианты работы, вне зависимости от направления деятельности.",
        ],
    },
    eng: {
        greeting: ["Hi! My name is", "Maxim Lalak."],
        about: "On this website I present the areas of work I do, as well as the results of my activities. If you want to cooperate, you can contact me by any convenient way.",
        education:
            "Russian State University of Tourism and Service, Lawyer, 2005-2010",
        experience: [
            "Professional experience: In-House Lawyer, 2012-2014",
            "Work experience in other fields: Radio presenter, 2015-2016; Secondary school teacher, 2008-2010",
            "Own business: Project management, entrepreneurial consultancy, full cycle of creating interactive entertainment projects.",
        ],
        occupation: {
            music: {
                myProjects: [
                    {
                        img: "projects/wavemeup.jpeg",
                        text: "WaveMeUp (metagrunge)",
                        link: "https://www.youtube.com/watch?v=spCX6BueUvo",
                    },
                    {
                        img: "projects/paranoid.jpg",
                        text: "Paranoid Park (chaotic hardcore)",
                        link: "https://www.youtube.com/watch?v=vRdtmslc_Xo",
                    },
                    {
                        img: "projects/raisdem.png",
                        text: "raizdem (electronic), KAROSHI (deftones core)",
                        link: "https://www.youtube.com/watch?v=ymzJwEoTDP4",
                    },
                    {
                        img: "projects/october.jpeg",
                        text: "Однажды в Октябре (emo, pop-punk).",
                        link: "https://www.youtube.com/watch?v=UIMSESUqs5E",
                    },
                ],
                description:
                    "I play music live, but some of the projects are studio. I make records by myself, try to study, work on improving quality and my own skills in that direction.",

                podcast: [
                    {
                        name: "The podcast 'Vocal Norms?'",
                        link: "https://music.yandex.ru/album/25333859",
                    },

                    {
                        name: "Dmitry More's YouTube channel, with an audience of over 100,000 ",
                        link: "https://www.youtube.com/@DmitryMore. ",
                    },
                ],
            },

            copywriting: [
                "I work in the Hangover project, which is a video blog and a social networking community with an aggregate audience of 35,000 people. I write texts about new releases and other subjects connected with music, literature and cinema. I also do newsgathering and help write scripts for weekly podcasts. ",
                "I write texts for the Azalia Now e-commerce blog.",
            ],
            testing: [
                "I am engaged in manual testing of websites, as well as testing of mobile applications for Apple via TestFlight. I make necessary documentation, maintain timely reporting, in accordance with company requirements",
            ],
        },
        contact: [
            "In all represented areas I try to develop and constantly improve the quality of work; Always looking for creative and creative approach, trying to create a new. I am passionate and give myself to the cause 100%.",
            "If you would like to collaborate, I am willing to consider any options for work, regardless of the direction.",
        ],
    },
};
